	...
	@:noCompletion private inline function advance():Void
	{
        switch (prev)
        {
            case BEGIN_BITMAP_FILL:
                oPos = (oPos + 2); // bitmap, matrix
                bPos = (bPos + 2); // repeat, smooth
            ...
            case BEGIN_GRADIENT_FILL:
                oPos = (oPos + 4); // type, matrix, spreadMethod, interpolationMethod
                iiPos = (iiPos + 2); // colors, ratios
                ffPos = (ffPos + 1); // alphas
                fPos = (fPos + 1); // focalPointRatio
        
            case BEGIN_SHADER_FILL:
                oPos = (oPos + 1); // shaderBuffer
        
            case CUBIC_CURVE_TO:
                fPos = (fPos + 6); // controlX1, controlY1, controlX2, controlY2, anchorX, anchorY
        
            case CURVE_TO:
                fPos = (fPos + 4); // controlX, controlY, anchorX, anchorY
        
            case DRAW_CIRCLE:
                fPos = (fPos + 3); // x, y, radius
        
            case DRAW_ELLIPSE:
                fPos = (fPos + 4); // x, y, width, height
        
            case DRAW_QUADS:
                oPos = (oPos + 3); // rects, indices, transforms
        
            case DRAW_RECT:
                fPos = (fPos + 4); // x, y, width, height
        
            case DRAW_ROUND_RECT:
                fPos = (fPos + 5); // x, y, width, height, ellipseWidth
                oPos = (oPos + 1); // ellipseHeight
        
            case DRAW_TRIANGLES:
                oPos = (oPos + 4); // vertices, indices, uvtData, culling
            ...
            case LINE_STYLE:
                oPos = (oPos + 4); // thickness, scaleMode, caps, joints
                iPos = (iPos + 1); // color
                fPos = (fPos + 2); // alpha, miterLimit
                bPos = (bPos + 1); // pixelHinting
