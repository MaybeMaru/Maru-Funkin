...
class Note extends FlxSpriteExt
{
	...
	override function update(elapsed:Float)
	{
		...
		{ // Move to strum
			...
			if (isSustainNote)
			{ // Get if the sustain is between pressing bounds
				angle = approachAngle;
				...
				if (Conductor.songPosition >= strumTime && pressed)
				{ // Sustain is being pressed
					setSusPressed();
				}
			}
			else
			...
			active = Conductor.songPosition < (strumTime + initSusLength +
				getPosMill(NoteUtil.swagHeight * 2)); // (getPosMill(height * Math.max(scale.y, 1)) + 100));
		}
	}
	...
	public function drawSustain(forced:Bool = false, ?newHeight:Int)
	{
		...
		{
			if (forced || (_height > height))
			{ // New graphic
				var key:String = 'sus$noteData-$_height-$skin';
				if (FlxG.bitmap.checkCache(key))
				{ // Save on drawing the graphic more than one time?
	...
	public function getPosMill(pos:Float):Float
