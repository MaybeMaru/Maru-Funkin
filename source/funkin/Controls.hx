...
class Controls
{
	...
	inline private static function checkKey(bindType:String = '', bindArray:Array<FlxKey>, gamepadBindArray:Array<FlxGamepadInputID>):Bool
	{
		if (inGamepad())
		{
			...
			{
				case 'R':
					return gamepad.anyJustReleased(gamepadBindArray); //  Release Key
				case 'P':
					return gamepad.anyJustPressed(gamepadBindArray); //  Press Key
				default:
					return gamepad.anyPressed(gamepadBindArray); //  Hold Key
			}
		}
		else
		{
			...
			{
				case 'R':
					return FlxG.keys.anyJustReleased(bindArray); //  Release Key
				case 'P':
					return FlxG.keys.anyJustPressed(bindArray); //  Press Key
				default:
					return FlxG.keys.anyPressed(bindArray); //  Hold Key
			}
		}
	}
	...
	inline public static function addBinding(bind:String, keys:Array<String>, gamepadKeys:Array<String>):Void
	{
		...
		if (!controlBindings.exists(bind))
			controlBindings.set(bind, keys);
		if (!controlGamepadBindings.exists(bind))
	...
	{
		return [
			getKey('NOTE_LEFT$type'),
			getKey('NOTE_DOWN$type'),
			getKey('NOTE_UP$type'),
			getKey('NOTE_RIGHT$type')
		];
	}
}
