...
class TitleState extends MusicBeatState
{
	...
	function makeIntroShit(index:Int):Void
	{
		...
		if (nuggets.create != null)
			makeText(nuggets.create);
		if (nuggets.add != null)
			addText(nuggets.add);
		if (nuggets.makeRandom != null)
			makeText([curWacky[nuggets.makeRandom]]);
		if (nuggets.addRandom != null)
			addText(curWacky[nuggets.addRandom]);
		if (nuggets.clear)
			clearText();
		if (nuggets.skip)
			skipIntro();
