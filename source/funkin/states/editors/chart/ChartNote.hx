...
class ChartNote extends Note
{
	...
	public function init(_time, _data, _xPos, _yPos, _sus, _skin, forceSus = false, ?_parent:Note)
	{
		...
		if (isSustainNote)
		{
			...
			var _height = Math.floor(((FlxMath.remapToRange(_sus, 0, Conductor.stepCrochet * 16, 0, ChartGrid.GRID_SIZE * Conductor.STEPS_SECTION_LENGTH))
				+ ChartGrid.GRID_SIZE / 2) / _scale);
