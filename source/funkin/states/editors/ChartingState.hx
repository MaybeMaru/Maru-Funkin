...
class ChartingState extends MusicBeatState
{
	...
	override function create()
	{
		...
		bg._dynamic.update = function(elapsed)
		{
			if (bg.scale.x <= 1.11)
				return;
			bg.scale.set(CoolUtil.coolLerp(bg.scale.x, 1.1, 0.25), CoolUtil.coolLerp(bg.scale.y, 1.1, 0.25));
		}
		add(bg);
		...
		{
			if (ChartGrid.getGridOverlap(FlxG.mouse, mainGrid.grid))
			{ // display dummy arrow
		...
		songTxt._dynamic.update = function(elapsed)
		{
			var info = "Time: "
				...
				+ " / "
				+ FlxStringUtil.formatTime(Conductor.inst.length / 1000, true)
				+ "\n"
				+ "Step: "
				+ Math.max(0, curStep)
				+ "\n"
				...
				+ "\n"
				+ "Section: "
				+ Math.max(0, curSection)
				+ "\n\n"
				+ "Position: "
				+ Math.floor(Conductor.songPosition)
				+ "\n"
				+ "BPM: "
				+ Conductor.bpm;
			songTxt.text = info;
		...
		mainGrid.notesGroup.forEachAlive(function(note:ChartNote)
		{
			if (note.strumTime + 0.1 <= Conductor.songPosition)
			{
				if (note.color == FlxColor.WHITE)
				{
					...
					if (playing && tabs.check_hitsound.checked)
						CoolUtil.playSound('chart/hitclick', 1, 1);
				}
				note.color = gray;
				if (note.childNote != null)
					note.childNote.color = gray;
			}
			else
			{
				note.color = FlxColor.WHITE;
				if (note.childNote != null)
					note.childNote.color = FlxColor.WHITE;
			}
		});

		if (SONG.notes[sectionIndex] == null)
			return;
	...
	function checkBPM(updateSec:Bool = false)
	...
	public function changeSection(change:Int = 0)
	{
		...
		Conductor.autoSync();
		if (SONG.notes[sectionIndex] == null)
			SONG.notes[sectionIndex] = Song.getDefaultSection(); // Make new section
		checkBPM();
	...
	{
		if (selectedNote == null || selectedNoteObject == null)
			return;
	...
	function moveTime(elapsed:Float = 0)
	{
		...
		if (Conductor.songPosition >= getSecTime(sectionIndex + 1))
			changeSection(1); // Go to next section
		if (getTimeY(Conductor.songPosition - getSecTime(sectionIndex)) < 0 && sectionIndex > 0)
		{ // Go to prev section
			changeSection(-1);
			Conductor.songPosition = getSecTime(sectionIndex + 1) - 1;
			Conductor.autoSync();
		}
		if (Conductor.songPosition > Conductor.inst.length)
		{ // End song
	...
	function keys()
	{
		...
		if (FlxG.keys.justPressed.D || FlxG.keys.justPressed.A || FlxG.keys.pressed.W || FlxG.keys.pressed.S)
		{
			...
			if (FlxG.keys.justPressed.D)
				changeSection(1 * mult);
			if (FlxG.keys.justPressed.A)
				changeSection(-1 * mult);
			if (FlxG.keys.pressed.W)
				moveTime(-FlxG.elapsed * mult);
		...
		if (FlxG.keys.justPressed.E || FlxG.keys.justPressed.Q)
		{
			if (FlxG.keys.justPressed.E)
				changeNoteSus(Conductor.stepCrochet);
	...
	function mouse()
	{
		...
		{
			if (FlxG.mouse.overlaps(mainGrid.notesGroup))
			{ // Remove notes
				mainGrid.notesGroup.forEachAlive(function(note:ChartNote)
				{
	...
	function set_selectedNoteObject(value)
	{
		...
		else if (value == null && selectedNoteObject != null)
			selectedNoteObject.blend = NORMAL;
		else if (value != null && selectedNoteObject == null)
			value.blend = ADD;
	...
	{
		return FlxMath.remapToRange(strumTime, 0, Conductor.STEPS_SECTION_LENGTH * Conductor.stepCrochet, 0,
			ChartGrid.GRID_SIZE * Conductor.STEPS_SECTION_LENGTH);
	}
