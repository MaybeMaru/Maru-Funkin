...
class PlayState extends MusicBeatState
{
	...
	public var notes(get, never):FlxTypedGroup<Note>;

	function get_notes()
		return notesGroup.notes;

	public var unspawnNotes(get, never):Array<Note>;

	function get_unspawnNotes()
		return notesGroup.unspawnNotes;

	public var strumLine(get, never):FlxSprite;

	function get_strumLine()
		return notesGroup.strumLine;

	public var holdingArray(get, never):Array<Bool>;

	function get_holdingArray()
		return notesGroup.holdingArray;

	public var controlArray(get, never):Array<Bool>;

	function get_controlArray()
		return notesGroup.controlArray;

	public var strumLineNotes(get, never):FlxTypedGroup<NoteStrum>;

	function get_strumLineNotes()
		return notesGroup.strumLineNotes;

	public var playerStrums(get, never):FlxTypedGroup<NoteStrum>;

	function get_playerStrums()
		return notesGroup.playerStrums;

	public var opponentStrums(get, never):FlxTypedGroup<NoteStrum>;

	function get_opponentStrums()
		return notesGroup.opponentStrums;

	public var strumLineInitPos(get, never):Array<FlxPoint>;

	function get_strumLineInitPos()
		return notesGroup.strumLineInitPos;

	public var playerStrumsInitPos(get, never):Array<FlxPoint>;

	function get_playerStrumsInitPos()
		return notesGroup.playerStrumsInitPos;

	public var songSpeed(get, never):Float;

	function get_songSpeed()
		return NotesGroup.songSpeed;

	public var opponentStrumsInitPos(get, never):Array<FlxPoint>;

	function get_opponentStrumsInitPos()
		return notesGroup.opponentStrumsInitPos;

	public var grpNoteSplashes(get, never):FlxTypedGroup<NoteSplash>;

	function get_grpNoteSplashes()
		return notesGroup.grpNoteSplashes;

	public var curSong(get, never):String;

	function get_curSong()
		return notesGroup.curSong;

	public var generatedMusic(get, never):Bool;

	function get_generatedMusic()
		return notesGroup.generatedMusic;

	public var inst(get, never):FlxSound;

	function get_inst()
		return Conductor.inst;

	public var vocals(get, never):FlxSound;

	function get_vocals()
		return Conductor.vocals;

	public var skipStrumIntro(get, set):Bool;

	function get_skipStrumIntro()
		return notesGroup.skipStrumIntro;

	function set_skipStrumIntro(value)
		return notesGroup.skipStrumIntro = value;

	public var inBotplay(get, set):Bool;

	function get_inBotplay()
		return notesGroup.inBotplay;

	function set_inBotplay(value)
		return notesGroup.inBotplay = value;
	...
	override public function create():Void
	{
		...
		notesGroup.goodNoteHit = function(note:Note)
		{
			...
			notesGroup.inBotplay ? notesGroup.playStrumAnim(note.noteData + Conductor.NOTE_DATA_LENGTH,
				'confirm') : notesGroup.playerStrums.members[note.noteData].playStrumAnim('confirm', true);
		...
		notesGroup.opponentNoteHit = function(note:Note)
		{
			...
			if (!getPref('vanilla-ui'))
				notesGroup.playStrumAnim(note.noteData % Conductor.NOTE_DATA_LENGTH);
		...
		healthBar = new FlxBar(healthBarBG.x + 6, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 12), Std.int(healthBarBG.height - 8), this,
			'health', 0, 2);
		...
		for (i in ['botplay', 'practice'])
			watermark.addAnim(i, i.toUpperCase(), 24, true);
		watermark.playAnim(notesGroup.inBotplay ? 'botplay' : 'practice');
		watermark.setScale(SkinUtil.curSkinData.scale * 0.7);
		watermark.x -= watermark.width * 1.2;
		watermark.y -= watermark.height * (getPref('downscroll') ? -0.2 : 1.2);
	...
	function createDialogue():Void
	{
		...
		if (dialogueBox != null && dialogueBox.skipIntro)
			black.alpha = 0;
	...
	function quickDialogueBox():Void
	{
		if (dialogueBox == null)
		{
			...
			{
				case 'pixel':
					dialogueBox = new PixelDialogueBox();
				default:
					dialogueBox = new NormalDialogueBox();
			}
		}
	...
	function startCountdown():Void
	{
		...
		startTimer = new FlxTimer().start(Conductor.crochetMills, function(tmr:FlxTimer)
		{
			...
			if (swagCounter > 0)
			{
				...
				FlxTween.tween(countdownSpr, {alpha: 0}, Conductor.crochetMills, {ease: FlxEase.cubeInOut, onComplete: function(twn:FlxTween)
				{
	...
	function updateScore():Void
	{
		...
		scoreTxt.text = 'Score: $songScore / Accuracy: ${(noteCount > 0) ? '$songAccuracy%' : ''} [$songRating] / Misses: $songMisses';
	...
	override public function update(elapsed:Float):Void
	{
		...
		if (FlxG.keys.justPressed.NINE)
		{
			...
			{
				case 'bf-old':
					iconP1.makeIcon('bf-older');
					iconP2.makeIcon('dad-older');
				case 'bf-older':
					iconP1.makeIcon(boyfriend.icon);
					iconP2.makeIcon(dad.icon);
				default:
					iconP1.makeIcon('bf-old');
					iconP2.makeIcon('dad');
			}
		}
	...
	{
		if (!notesGroup.generatedMusic || curSectionData == null)
			return;
		...
		var intendedPos:Float = mustHit ? bfMidpointX - boyfriend.camOffsets.x - stageJsonData.bfCamOffsets[0] : dadMidpointX
			- dad.camOffsets.x
			- stageJsonData.dadCamOffsets[0];
	...
	public function switchChar(type:String, newCharName:String)
	{
		...
		if (lastAnim != null)
			newChar.playAnim(lastAnim, true, false, lastFrame);
		if (targetIcon != null)
			targetIcon.makeIcon(newChar.icon);
