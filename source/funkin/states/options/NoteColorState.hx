...
class NoteColorState extends MusicBeatState
{
    ...
    function changeNoteSelection(change:Int = 0):Void {
        ...
        curNote += change;
        if (curNote > notesGroup.members.length-1) curNote = 0;
        if (curNote < 0) curNote = notesGroup.members.length-1;

        for (i in 0...notesGroup.members.length)
        {
            notesGroup.members[i].alpha = 0.5;
            if (i == curNote)   notesGroup.members[i].alpha = 1;
        }
    }
    ...
    function changeRowSelection(change:Int = 0):Void {
        ...
        for (i in 0...optionsGroup.members.length)
        {
            ...
            for (crap in guh) idShit.push(Std.parseInt(crap));
    ...
    override function update(elapsed:Float)
    {
        ...
        if (getKey('UI_LEFT-P') || getKey('UI_RIGHT-P'))
		{
            ...
            {
                if (getKey('UI_LEFT-P'))    changeNoteSelection(-1);
                if (getKey('UI_RIGHT-P'))   changeNoteSelection(1);
            }
		}

        if (getKey('UI_UP-P') || getKey('UI_DOWN-P') && inNote)
        {
            if (getKey('UI_UP-P'))    changeRowSelection(-1);
            if (getKey('UI_DOWN-P'))  changeRowSelection(1);
        }
