...
class FlxRuntimeShader extends FlxShader
{
	...
	private var ___fieldList:Array<String> = null;

	private function _thisHasField(name:String)
	{
		// Reflect.hasField(this, name) is REALLY expensive so we use a cache.
		if (___fieldList == null)
		{
			___fieldList = Reflect.fields(this).concat(Type.getInstanceFields(Type.getClass(this)));
		}
		return ___fieldList.indexOf(name) != -1;
	}
	...
	private override function __processGLData(source:String, storageType:String):Void
	{
		...
		while (regex.matchSub(source, lastMatch))
		{
			...
			if (StringTools.startsWith(type, "sampler"))
			{
				...
				if (__isGenerated && _thisHasField(name))
