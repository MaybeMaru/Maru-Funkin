...
function beatHit(curBeat)
{
	...
	{
		case 128:
			addBlammedTransition();
		case 126:
			startBlammedTransition();
	}
...
function startBlammedTransition()
{
	...
	var overlayTrainBG:FunkinSprite = new FunkinSprite('', [overlayTrain.x, overlayTrain.y],
		[0, 0]).makeGraphic(overlayTrain.width / 1.35, overlayTrain.height / 1.4, FlxColor.BLACK);
...
{
	for (i in ['blammedOverlay', 'tunnelBG', 'tunnelLights', 'tunnelLightsBlur'])
		getSpr(i).visible = false;
	for (i in ['cityLoop0', 'cityLoop1'])
		getSpr(i).visible = true;
	...
	for (i in ['phillyWindow', 'phillyWindowBlur', 'phillyCity'])
		getSpr(i).y += 2000;
